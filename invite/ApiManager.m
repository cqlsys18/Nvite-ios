         ...
         progress:^(NSProgress * _Nonnull uploadProgress) {
          ...
          success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              ...
              if (statusCode == 403) {
                  ...
                  NSLog(@"error: %@",[response.allHeaderFields objectForKey:@"message"]);
                  if ([[response.allHeaderFields objectForKey:@"message"] isEqualToString:@"USER NOT EXIST"]) {
                      [[NSNotificationCenter defaultCenter] postNotificationName:
                       @"LOGIN" object:nil userInfo:nil];
                  }
                  else{
                      HIDE_ACTIVITE_INDICATOR;
                      completionBlock(false, [response.allHeaderFields objectForKey:@"message"],nil);
                  }
    ...
    uploadTask = [manager
                  ...
                  completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                      ...
                      if (error) {
                          ...
                          NSLog(@"Error: %@", error.localizedDescription);
                          completionBlock(false, error.localizedDescription,responseObject);
...
-(void)apiCallWithImage:(NSString *)_urlString parameterDict:(NSDictionary *)_parameterDict imageDataDictionary:(NSDictionary *)_imageDatadictionary videoDataDictionary:(NSDictionary *)_videoDatadictionary CompletionBlock :(DictionaryResponse) completionBlock
{
    ...
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:_urlString parameters:_parameterDict constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        for (NSString* key in _videoDatadictionary){
            ...
            [formData appendPartWithFileData:[_videoDatadictionary objectForKey:key] name:key fileName:fileName1 mimeType:@"image/jpeg"];
