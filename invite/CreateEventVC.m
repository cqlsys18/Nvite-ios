...
- (void)viewDidLoad {
    ...
    GroupPicker = [[UIPickerView alloc] init];
    GroupPicker.delegate    = self;
    GroupPicker.dataSource  = self;
    GroupPicker.tag         = 1;
    eventTypeTF.inputView = GroupPicker;
    
    UIPickerView *GroupPicker1;
    GroupPicker1 = [[UIPickerView alloc] init];
    GroupPicker1.delegate    = self;
    GroupPicker1.dataSource  = self;
    GroupPicker1.tag         = 2;
    eventSubType.inputView = GroupPicker1;
    
    typeArray = [NSArray arrayWithObjects:@{@"type" : @"Public" , @"id":@"4"},@{@"type" : @"Private" , @"id":@"1"}, nil];
    
    subTypeArray = [NSArray arrayWithObjects:@{@"type" : @"Closed" , @"id":@"1"},@{@"type" : @"Private Plus One" , @"id":@"2"},@{@"type" : @"Open" , @"id":@"3"}, nil];
...
- (IBAction)createEventBtnAction:(id)sender {
    ...
    if ([[eventNAmeTF.text stringByReplacingOccurrencesOfString:@" " withString:@""] isEqualToString:@""]) {
        ...
        if ([selectedTypStr isEqualToString:@"4"]) {
                 ...
                 else
                 {
                     ...
                     [[ApiManager sharedInstance] apiCallWithImage:urlString parameterDict:dict imageDataDictionary:imgDict  CompletionBlock:^(BOOL success, NSString *message, NSDictionary *dictionary)  {
                         ...
                         else
                         {
                             ...
                             {
                                 UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:@"Event Created Successfully!" preferredStyle:UIAlertControllerStyleAlert];
            ...
            EventInvitesVC *view    = [self.storyboard instantiateViewControllerWithIdentifier:@"EventInvitesVC"];
            view.dataDict       = [[NSMutableDictionary alloc] initWithDictionary:dict];
            view.imgDataDict    = [[NSMutableDictionary alloc] initWithDictionary:imgDict];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow: (NSInteger)row inComponent:(NSInteger)component {
    ...
    if (pickerView.tag == 1) {
        eventTypeTF.text    = [typeArray[row] objectForKey:@"type"];
        selectedTypStr      = [typeArray[row] objectForKey:@"id"];
        
        [self updateView:row];
    }
    else {
        eventSubType.text   = [subTypeArray[row] objectForKey:@"type"];
        selectedTypStr      = [subTypeArray[row] objectForKey:@"id"];
    }
...
- (NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (pickerView.tag == 1) {
        return [typeArray[row] objectForKey:@"type"];
    }
    return [subTypeArray[row] objectForKey:@"type"];
}
...
#pragma mark -  CustomMethodToUpdateView


- (void) updateView : (long) selectedRow{

    if (selectedRow == 0) {
        
        createEventBTn.selected = false;
        
        eventTypeV.hidden = true;
        topSpaceDateTFConst.constant = 10;
    }
    else {
        
        createEventBTn.selected = true;
        eventTypeV.hidden = false;
        topSpaceDateTFConst.constant = eventTypeV.frame.size.height+10;
    }
